...
## 9. Integração com o Frontend (React Native / Expo)

### 2026-05-15 — Criação das Telas de Autenticação, Refinamentos de UI e Integração de APIs

- **Implementação do MVVM:** O fluxo de login e cadastro foi construído utilizando o padrão MVVM, dividindo a interface (`LoginView.tsx`, `RegisterView.tsx`) da lógica de negócios e requisições (`useLoginViewModel.ts`, `useRegisterViewModel.ts`).
- **Integração das APIs:** 
  - O endpoint de login `POST /auth/login` foi integrado. O envio de dados (`email` e `senha`) é realizado via `fetch` de forma assíncrona apontando para a base URL da API no Render.
  - O endpoint de cadastro `POST /usuarios` foi integrado, passando o payload (`nome`, `email` e `senha`).
  - O gerenciamento de estado de carregamento (`isLoading`) foi implementado para exibir o `ActivityIndicator` nativo nos botões, bloqueando reenvios durante a espera pela rede.
- **Navegação Global e Fluxo:** Adicionado um controle de rotas minimalista no `App.tsx` capaz de navegar entre `login`, `register` e a nova `home`. Após receber um sucesso HTTP (200/201) da API de autenticação, o usuário é direcionado à `HomeView.tsx` limpa recém-criada.
- **Melhorias Premium de UI e Tratamento de Erros:**
  - Foi abolido o `Alert.alert` genérico para falhas de formulário, sendo trocado por um estado `error` que renderiza um componente de aviso em HTML/JSX customizado (com ícone e tema vermelho).
  - Foi implementado um sistema de **animação de tremedeira (Shake)** através do `Animated` (React Native) associado ao Card principal; sempre que um erro retorna da API, o formulário vibra horizontalmente de maneira suave e profissional.
  - Adicionado o uso das bibliotecas robustas `react-native-safe-area-context` e `react-native-keyboard-aware-scroll-view`, substituindo as opções "deprecated" ou engessadas nativas para melhor manipulação do teclado e das bordas/entalhes do celular.
